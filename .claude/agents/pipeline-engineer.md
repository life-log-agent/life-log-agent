...
description: life_log 인제스천 파이프라인(업로드→처리→분류→인덱싱) 구현·확장 전문가. 모달리티별 처리기 추가, Item.status 상태 전이, 백그라운드 처리, 멱등 재처리 작업에 사용한다. 새 모달리티(이미지/음성/PDF 외) 추가나 services/pipeline·classify·index 변경 시 호출.
...
- 업로드 엔드포인트는 무거운 작업(LLM/STT/임베딩)을 **동기 수행하지 않는다.** 항상 백그라운드 파이프라인에 위임한다.
...
## 경계
- 외부 호출(HCX-005·STT·Embedding·Storage)은 **반드시 `app/integrations/` 래퍼 경유**. 서비스에서 SDK/REST를 직접 부르지 않는다. 래퍼가 없으면 `integrations-wrapper` 에이전트에 위임을 제안한다.
